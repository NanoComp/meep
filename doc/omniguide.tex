...
int main(int argc, char **argv) {
  ...
  mat ma(volcyl(rcore + num_layers + 0.3, 0.0, a), guided_eps);
  const char *dirname = make_output_directory(argv[0]);
  FILE *bands = create_output_file(dirname,"funbands");
\end{comment}
...
\begin{comment}
  {
    ...
    for (double k=0.0;k<0.351 && !interrupt;k+=0.05) {
      ...
      f.prepare_for_bands(vec(2.801,0.0), ttot, .40, 300, 0.0);
      f.prepare_for_bands(vec(1.801,0.0), ttot, .40, 300, 0.0);
      const double stoptime = f.time() + ttot;
      while (f.time() < stoptime && !interrupt) {
        f.record_bands();
        f.step();
      }
      f.output_bands(bands, "bands", 40);
      f.grace_bands(&g, 40);
    }
  }
\end{comment}
