...
\begin{comment}
  {
    ...
    g.new_set();
    g.set_legend("m = 0, TM");
    for (double k=0.0;k<0.51 && !interrupt;k=(k)?k*sqrt(sqrt(10)):0.05) {
      char k_and_m[10];
      ...
      f.initialize_with_n_tm(12);
      double next_slice_time = 0.0;
      while (f.is_phasing() && !interrupt) {
        if (f.time() >= next_slice_time && 0) {
          next_slice_time += 100.0;
          printf("Phasing in time %lg...  ", f.time());
          printf("energy is %lg\n", f.total_energy());
          f.output_slices(k_and_m);
        }
        f.step();
      }
      f.prepare_for_bands(0, ttot, .5, 10000, 0.0);
...
epsilon as usual.
\begin{verbatim}
      f.initialize_with_n_te(12);
      if (m) f.initialize_with_n_tm(12);
\end{verbatim}
\begin{comment}
...
\begin{verbatim}
      f.prepare_for_bands(0, ttot, .5, 10000, 1e-4);
      for (int t=0; t < ttot+1 && !interrupt; t++) {
        if (t % (500*a) == 0) {
          printf("Working on time %lg...  ", f.time());
          printf("energy is %lg\n", f.total_energy());
