...
Meep supports saturable gain and absorbing media through the use of a set of auxiliary equations which model both the polarization and inversion of the saturable medium. Meep's implementation of these auxiliary equations is similar to that described in [Optics Express, Vol. 19, pp. 1539, 2011](https://www.osapublishing.org/oe/abstract.cfm?uri=oe-19-2-1539), in which the polarization is treated using the oscillator model and the inversion is modeled as a set of rate equations for the population densities of the atomic levels of the saturable medium. Although the oscillator model is a second-order differential equation for the polarization, form used for saturable media is slightly different from the form given in the [introduction](Materials.md#material-dispersion), and instead is
...
excellent agreement is seen between all three numerical methods. Note that for the two-level atomic gain model used in this example, one can calculate $D_0$ as
...
Although the literature on lasers and saturable gain media does not have a universally accepted nomenclature for the various constants found within the media's equations, for the ease of convenience in attempting to convert between different variable naming conventions found in different references, note that
...
To convert Meep outputs to SALT units, use
...
and for a two level gain medium, $\gamma_\parallel = \gamma_{12} + \gamma_{21}$. Further reading on SALT, both as an introduction and for details on atomic media with an arbitrary number of levels can be found in [Optics Express, Vol. 23, pp. 6455, 2015](https://www.osapublishing.org/oe/abstract.cfm?uri=oe-23-5-6455). 
