...
**Note**: When downloading the source code from the [Meep Releases](https://github.com/NanoComp/meep/releases) page on Github, be sure to download the `meep-X.Y.Z.tar.gz` release file, *not* the `vX.Y.Z.tar.gz` file linked under "Source Code".  The latter is autogenerated by Github and is equivalent to the files you would get from a `git clone`, which require running a special `autogen.sh` file and special tools as explained under [Meep for Developers](Build_From_Source.md#meep-for-developers) below.  The `meep-X.Y.Z.tar.gz`, in contrast, includes all pre-generated source files, though you still need Unix compiler tools.
