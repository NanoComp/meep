...
This feature requires one to pass the `--enable-single` flag to the Meep `configure` script before compiling. In Python, you can determine whether the Meep module has been compiled with single-precision floating point using the boolean function `meep.is_single_precision()`. If you want to install both the single- and double-precision point versions, one option is to build the former using the `--enable-single` flag and then rename the `meep` module to `meep_single`.
