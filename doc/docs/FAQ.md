...
Incoherent emission from [excitons](https://en.wikipedia.org/wiki/Exciton) can be modeled using a [Monte Carlo method](https://en.wikipedia.org/wiki/Monte_Carlo_method) by taking an average of multiple separate runs involving a collection of point dipole sources with random phase set via the `amplitude` property of the [`Source`](Python_User_Interface.md#source). For an example, see [Applied Physics Letters, 106, 041111, 2015](https://aip.scitation.org/doi/abs/10.1063/1.4907253) ([pdf](http://ab-initio.mit.edu/~oskooi/papers/Oskooi15_oled.pdf)) and the associated [simulation script](http://www.simpetus.com/projects.html#meep_oled).
