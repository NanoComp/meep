...
Note that a free-charge ($\pho_f$) current in a polarizable medium such as a dielectric material (including vacuum) creates [bound charges](https://en.wikipedia.org/wiki/Maxwell%27s_equations#Bound_charge_and_current) which in turn can act as a secondary current source. In an FDTD simulation, these bound charges $\pho_b$ (which are equivalent to a polarization field $\vec{P}$ or [electric dipole moment](https://en.wikipedia.org/wiki/Electric_dipole_moment)) may make it seem that the current source never turns off because the fields do not decay away to zero. To ensure that the net dipole moment is zero when the source turns off, the source should be the *integral* of $\mathcal{J}$ rather than just $\mathcal{J}$. This feature is enabled in Meep by setting the `is_integrated` parameter of the [`GaussianSource`](Python_User_Interface.md#gaussiansource) class object to `True` (default is `False`).
