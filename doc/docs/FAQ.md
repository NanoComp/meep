...
when the current has turned off, it corresponds to an electrostatic free-charge distribution that is left in the system, which produces electrostatic electric fields.   For example, the Gaussian current source in Meep can have a slightly nonzero integral, and the signature of this is that a slight electrostatic field may be left over after
the simulation has run for a long time.   This has no affect on Fourier-transformed (DFT) field quantities in Meep at nonzero fields, but it can occasionally be an annoyance.
In order to get rid of this residual electrostatic field, you can set the `is_integrated` parameter of the source time-dependence (e.g. [`GaussianSource`](Python_User_Interface.md#gaussiansource)) to `True` (default is `False`): what this does is to make the source term the polarization $\mathbf{P}$ (the *integral* of $\mathbf{J}$, which enters Maxwell's equations via the relationship between $\mathbf{E}$ and $\mathbf{D}$, and thus ensures that when the source is zero, the free
