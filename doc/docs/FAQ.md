...
### Is it possible to model a dipole emitter in a resonant cavity for the case in which the cavity lifetime is longer than the coherence time of the dipole?

Yes. This can be simulated in Meep directly with a stochastic dipole source (i.e., random phase) having a specified correlation function. There are various semi-analytical approaches. For example, if the cavity lifetime (which is proportional to the [quality factor](https://en.wikipedia.org/wiki/Q_factor#Optical_systems) $Q$) is longer than the coherence time of the dipole emitter (e.g., [quantum dot](https://en.wikipedia.org/wiki/Quantum_dot), [quantum well](https://en.wikipedia.org/wiki/Quantum_well), [exciton](https://en.wikipedia.org/wiki/Exciton), etc.) this corresponds to a resonance system that is narrower than the source spectrum and the cavity effectively only "sees" a small fraction of the source power. In a linear system, the nice thing is that you can compute the system response (e.g., the [transfer function](https://en.wikipedia.org/wiki/Transfer_function) of the source into some output channel; such as the [local density of states](Python_User_Interface.md#ldos-spectra) (LDOS)) *once* via deterministic calculations over a sufficiently broad bandwidth, and then afterwards put in whatever source spectrum or time dependence you want within the bandwidth. For examples, see [Tutorial/Local Density of States](Python_Tutorials/Local_Density_of_States.md) and [Light Extraction Efficiency of Organic Light Emitting Diodes](http://www.simpetus.com/projects.html#meep_oled) (OLEDs).

Note: if the system is linear, the power emitted by the dipole (i.e., the LDOS spectra) is the same whether the dipole/sources at different frequencies are coherent or not. You can therefore put in any source spectrum you want, even an ultra-wide bandwidth Gaussian pulse, and rescale to any desired spectrum in post processing.
...
Note: in this example, the beam waist is at the source position (i.e., top center of the cell). If you want the beam waist to be at a position other than the position of the source, you need to adjust the *phase* of the beam accordingly. If you assume you have a Gaussian beam profile with zero phase at some plane y=y<sub>0</sub>, then you can work out the beam profile (including phase) at any other plane y=y<sub>1</sub> by taking the Fourier transform and looking at the propagation of each planewave component, and then inverse Fourier transforming. In this way, you can work out the desired source profile at any plane y=y<sub>1</sub> to get a Gaussian beam waist at y=y<sub>0</sub>.
