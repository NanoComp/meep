...
Note: for broadband calculations involving Fourier-transformed fields or [Harminv](Python_User_Interface.md#harminv), the frequency bandwidth of the Gaussian source and the fields monitor should be the same. This is because the bandwidth of the Gaussian source (`fwidth` property) is [proportional to its full-width at half maximum](https://en.wikipedia.org/wiki/Gaussian_function#Properties) at which its amplitude is non-zero. However, a Gaussian source with very narrow bandwidth could introduce [floating-point errors](https://en.wikipedia.org/wiki/Round-off_error) when computing the [transmittance/reflectance spectra](Introduction/#transmittancereflectance-spectra) due to normalization by the Fourier-transformed fields with values close to zero.
