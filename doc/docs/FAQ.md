...
Regarding (3), if the fields are still blowing up when overlapping the susceptibility material with a PML or Absorber, consider *removing* the absorbing boundary layer on that side to preclude its overlap with the material. If the material is strongly reflective or absorptive, there are likely no transmitted fields. In this case, a PML or Absorber is unnecessary.
