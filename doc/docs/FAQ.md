...
Instability in the fields is likely due to one of three causes: (1) [PML](Python_User_Interface.md#pml) overlapping lossy materials involving a [Drude-Lorentzian susceptibility](Python_User_Interface.md#lorentziansusceptibility) (fix: replace the PML with an [Absorber](Python_User_Interface.md#absorber)), (2) the `frequency` of a `LorentzianSusceptibility` term is too high relative to the grid discretization (fix: increase the `resolution` and/or turn off subpixel smothing via `eps_averaging=False` and/or reduce the `Courant` factor from its default of `0.5`), or (3) a material with a wavelength-independent negative real permittivity (fix: [fit the permittivity to a broadband Drude-Lorentzian susceptibility](#how-do-i-import-n-and-k-values-into-meep)).
