...
For a potential improvement in [load balancing](FAQ.md#should-i-expect-linear-speedup-from-the-parallel-meep), you can try setting [`split_chunks_evenly=False`](Python_User_Interface.md#the-simulation-class) in the `Simulation` constructor. When there are a large number of chunks, this may also require *reducing* the default values for `subpixel_maxeval=100000` and `subpixel_tol=0.0001` to e.g. `10000` and `0.01`, respectively, to speed up the cell-partitioning algorithm (which otherwise can take several *hours* for a large heterogeneous 3d cell with more than ~100 chunks). These two parameters are used internally by the cell-partitioning algorithm as part of an adaptive quadrature scheme to compute the volume overlap of arbitrary subregions with the geometric objects over and over again when exploring a large space of possible cell partitions. For a technical description of the load-balancing features in Meep as well as some performance metrics from actual experiments, see [arXiv:2003.04287](https://arxiv.org/abs/2003.04287).
