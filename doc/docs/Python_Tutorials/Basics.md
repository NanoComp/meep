...
In this demonstration, we will verify this expression for the lossless dielectric sphere at a single wavelength (λ = 1 μm) by comparing with the analytic theory via PyMieScatt.
...
For `resolution = 25`, the error decreases (which is expected) to 1.5%.
```
