...
In the limit of infinite resolution, the discretization error is removed and the reflectance for $S_{11}$ and $S_{12}$ converge to their "true" values of ~10<sup>-6</sup> and ~10<sup>-8</sup>, respectively. (Note that the back-scattered fields in Port 2 are two orders of magnitude smaller than those in Port 1 because the input fields in the upper branch of the directional coupler must cross into the lower branch to reach Port 2.) In this example, $|S_{12}|^2$ requires a resolution of at least ~150 to minimize discretization errors. The discretization errors due to the eigenmode-coefficient extraction can be removed via a separate normalization run to compute the incident fields for just a straight waveguide (i.e., no taper/bend) which are then subtracted from the Fourier-transformed fields in Port 1 and 2 of the directional coupler. This procedure is similar to those involving [flux calculations](Basics.md#transmittance-spectrum-of-a-waveguide-bend). While this approach would enable more accurate results at *lower* resolutions, the additional calculation may be somewhat inconvenient. Also, for practical applications reflectance values less than 40 dB (e.g., telecom multi-path interference tolerances) are generally considered negligible. As in this example, the back-scattered fields are often sufficiently small that a separate run to obtain the incident fields is generally not necessary. However, there may be other applications, especially theoretical investigations, where trying to resolve such small reflections could be important. (Note that it is not possible to resolve reflections on the order of "machine precision" because floating-point precision will probably spoil even the normalization approach at that level.)
