...
This example involved computing the S-parameters at a single wavelength. For practical applications, however, a broad bandwidth spectrum is often desired. To compute the spectrum for a set of $N$ evenly spaced wavelengths $\lambda_i$ ($i = 1, 2, ... N$; $\lambda_i \lt \lambda_{i+1}$), one would typically specify the corresponding frequencies for the [DFT fields monitor](../Python_User_Interface.md#flux-spectra) by taking the inverse of each wavelength which would produce a set of $N$ *unevenly* spaced frequencies $f_i$ ($i = 1, 2, ... N$; $f_i \lt f_{i+1}$): $f_1 = 1/\lambda_N, f_2 = 1/\lambda_{N-1}, ... f_N = 1/\lambda_1$. Because of the Fourier Uncertainty Principle which requires that the minimum runtime of the simulation be equivalent to the inverse of the *smallest* frequency spacing $\Delta f_{min}$ that is being resolved, the simulation runtime may be chosen to be unnecessarily large only to resolve a single tiny frequency spacing (which could become expensive for large 3d simulations). A set of slightly coarser but *evenly* spaced frequencies can be specified using an alternative approach. The frequency resolution $\Delta f$ for a DFT field monitor given a wavelength resolution $\Delta\lambda$ around a wavelength $\lambda$ is $\Delta f = \Delta\lambda / \lambda^2$. This relationship is derived from the wave relation $f = c/\lambda$. For example, a 10 nm wavelength resolution around 1.31 μm is equivalent to a frequency resolution of ~0.0058. As a comparison, for 11 evenly spaced wavelengths in the silicon O-band spanning 1.26 to 1.36 μm (which is centered at 1.31 μm and has a wavelength spacing of 10 nm), the smallest frequency spacing is ~0.0054 which is roughly 7% smaller.
