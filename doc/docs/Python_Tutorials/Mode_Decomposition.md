...
The structure has mirror symmetry in the $y$ direction which can be exploited to reduce the computation size by a factor of two. To fully exploit this we also use the lower-level `add_flux` rather than `add_mode_monitor` (which is not optimized for symmetry), but this requires that we specify `eig_parity=mp.ODD_Z+mp.EVEN_Y` in the call to `get_eigenmode_coefficients`.
