...
The calculation of the reflection coefficient requires two separate runs: (1) a normalization run involving just the source medium ($n_1$) to obtain the incident fields, and (2) the main run including the interface whereby the incident fields from (1) are first subtracted from the monitor to obtain only the reflected fields. The mode coefficient in (2) divided by (1) is, by definition, the reflection coefficient. The phase is the [argument](https://en.wikipedia.org/wiki/Argument_(complex_analysis)) of the complex reflection coefficient. Note: there is a [convention](https://en.wikipedia.org/wiki/Total_internal_reflection#Phase_shifts), followed by Meep, used to define the phase which is based on the *sign* of the mode coefficients, in particular for the $P$ polarization.
