...
3. Compute the radial Poynting flux $P_i(\theta_i, \phi)$ for each of $N$ points $i = 0, 1, ..., N - 1$ on the circumference using $\Re\left[\vec{E}_{tot}(\theta_i, \phi) \times \vec{H}^*_{tot}(\theta_i, \phi)\right]$.
...
2. Obtain the powers $P(\theta)$ from these far fields by summing: $P(\theta) = \int_{0}^{2\pi} \left[ P_{m=0}(\theta) + 2\sum_{m=1}^{M} P_m(\theta)  \right] d\phi =  2\pi \Re\left[ \vec{E}_{m=0}(\theta) \times \vec{H}_{m=0}(\theta)^* + 2\sum_{m=1}^M \vec{E}_{m}(\theta) \times \vec{H}_{m}(\theta)^* \right]$.
3. Compute the fraction of the radial Poynting flux within an angular cone $\left[ \int_0^\theta P(\theta') d\theta' \right] / \left[ \int_0^{\pi/2} P(\theta') d\theta' \right]$ by some discretized integral, e.g. a [trapezoidal rule](https://en.wikipedia.org/wiki/Trapezoidal_rule) or something fancier.
