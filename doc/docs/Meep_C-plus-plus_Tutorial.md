...
We begin with a very brief outline of a Meep C++ program, with minimal explanations, leaving more details for the examples below and the [C++ reference page](Meep_C-plus-plus_Reference.md). Your C++ program should begin with:
...
2. Use the [w:pkg-config](https://en.wikipedia.org/wiki/pkg-config) program, which is installed by default on most GNU/Linux systems. Then, to compile a file `foo.cpp` using Meep, do:
