...
\epsfig{file=energy_cons-out/energy.eps,width=8.8cm}
\end{figure}
...
crop up at small $r$.
\begin{verbatim}
  mat_chunk ma(volcyl(1.0,20.0, a), eps);
\end{verbatim}
\begin{comment}
...
\begin{verbatim}
      ...
      g.output_out_of_order(1, f.time(), f.field_energy_in_box(f.v));
      g.output_out_of_order(2, f.time(), f.thermo_energy_in_box(f.v));
\end{verbatim}
\begin{comment}
    }
    f.step();
