...
int main(int argc, char **argv) {
  ...
  dft_flux fluxR = f.add_dft_flux_plane(box, freqs_re[0], freqs_re[0], 1);
  dpad = 0.02;
  volume_list line(volume(vec(0.5*d-dpad,-0.5*sy,0),vec(0.5*d-dpad,0.5*sy,0)),X);
  dft_force forceR = f.add_dft_force(&line, freqs_re[0], freqs_re[0], 1);
  f.zero_fields();
  f.t = 0;
  while (f.time() < T + T2) {
    if (f.t % 2000 == 0)
