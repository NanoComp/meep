...
{
  for(int n=0; n<N; n++)
   if ( ((float)array1[n]) !=  ((float)array2[n]) )
    return false;
  return true;
}
...
int main(int argc, char *argv[])
{
  ...
  if (has_direction(gv.dim,X)) 
   { vmin.set_direction(X,vxmin); vmax.set_direction(X,vxmax); }
  if (has_direction(gv.dim,Y)) 
   { vmin.set_direction(Y,vymin); vmax.set_direction(Y,vymax); }
  if (has_direction(gv.dim,Z)) 
   { vmin.set_direction(Z,vzmin); vmax.set_direction(Z,vzmax); }
