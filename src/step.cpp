...
void fields_chunk::step_source(field_type ft, bool including_integrated) {
  ...
  for (src_vol *sv = sources[ft]; sv; sv = sv->next) {
    ...
    /* In PML, current sources are transformed to update the U auxiliary field rather than D (or H);
       see issue #1046. */
    realnum *fc[2] = { f_u[c][0] ? f_u[c][0] : f[c][0], f_u[c][1] ? f_u[c][1] : f[c][1] };
    if ((including_integrated || !sv->t->is_integrated) && fc[0] &&
