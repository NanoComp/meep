...
bool pade_future_tail(const std::vector<pade_complex> &a, double omega, double t0,
  ...
  for (size_t q = L / 2; q != 0; --q) {
    ...
    const double denominator_threshold =
        std::max(128.0 * std::numeric_limits<double>::epsilon(),
                 pade_min_relative_denominator) *
        denominator_scale;
...
void dft_chunk::update_dft(double time) {
  ...
  PLOOP_OVER_IVECS(fc->gv, is, ie, idx) {
    ...
    if (numcmp == 2) {
      ...
      if (pade_samples)
        pade_history[idx_dft * pade_samples + pade_slot] =
            complex<realnum>(scale) * field_value;
    }
    else {
      realnum fr = f[0];
      for (int i = 0; i < Nomega; ++i)
...
dft_pade_error dft_chunk::get_pade_error() const {
  ...
  if (pade_diagnostic_generation != pade_generation) {
    ...
    if (have_previous) {
      ...
      for (size_t i = 0; i < Nomega; ++i)
        pade_error_cache.drift[i] =
            relative_norm(drift2[i], std::max(current2[i], previous2[i]));
    }
