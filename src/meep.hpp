...
class dft_energy {
public:
  // TODO: add dft_energy constructor
  dft_energy(dft_chunk *E_, dft_chunk *H_, dft_chunk *D_, dft_chunk *B_, std::vector<double> fs,
...
public:
  ...
  dft_force(dft_chunk *offdiag1_, dft_chunk *offdiag2_, dft_chunk *diag_, std::vector<double> fs,
            int Nf, const volume &where_);
...
public:
  ...
  // TODO: add dft_near2far constructor
  dft_near2far(dft_chunk *F, std::vector<double> fs, int Nf, double eps, double mu,
...
class dft_ldos {
public:
  // TODO: add dft_lods constructor
  dft_ldos(std::vector<double> freqs, int Nfreq);
...
class dft_fields {
public:
  // TODO: add dft_fields constructor
  dft_fields(dft_chunk *chunks, std::vector<double> freqs, int Nfreq, const volume &where);
...
public:
  ...
  dft_chunk *add_dft(component c, const volume &where, std::vector<double> freqs, int Nfreq,
  ...
  dft_chunk *add_dft_pt(component c, const vec &where, std::vector<double> freqs, int Nfreq);
  dft_chunk *add_dft_pt(component c, const vec &where, double freq_min, double freq_max, int Nfreq);
  // TODO: overload add_dft
  dft_chunk *add_dft(const volume_list *where, std::vector<double> freqs, int Nfreq,
                     bool include_dV = true);
  dft_chunk *add_dft(const volume_list *where, double freq_min, double freq_max, int Nfreq,
                     bool include_dV = true);
  void update_dfts();
  // TODO: overload add_dft_flux
  dft_flux add_dft_flux(const volume_list *where, std::vector<double> freqs, int Nfreq,
                        bool use_symmetry = true);
  dft_flux add_dft_flux(const volume_list *where, double freq_min, double freq_max, int Nfreq,
                        bool use_symmetry = true);
  // TODO: overload add_dft_flux
  dft_flux add_dft_flux(direction d, const volume &where, std::vector<double> freqs, int Nfreq,
                        bool use_symmetry = true);
  dft_flux add_dft_flux(direction d, const volume &where, double freq_min, double freq_max,
                        int Nfreq, bool use_symmetry = true);
  // TODO: overload add_dft_flux_box
  dft_flux add_dft_flux_box(const volume &where, std::vector<double> freqs, int Nfreq);
  dft_flux add_dft_flux_box(const volume &where, double freq_min, double freq_max, int Nfreq);
  // TODO: overload add_dft_flux_plane
  dft_flux add_dft_flux_plane(const volume &where, std::vector<double> freqs, int Nfreq);
  ...
  // TODO: overload add_mode_monitor
  dft_flux add_mode_monitor(direction d, const volume &where, std::vector<double> freqs, int Nfreq);
  dft_flux add_mode_monitor(direction d, const volume &where, double freq_min, double freq_max,
                            int Nfreq);


  // TODO: overload add_dft_fields
  dft_fields add_dft_fields(component *components, int num_components, const volume where,
                            std::vector<double> freqs, int Nfreq, bool use_centered_grid=true);
  dft_fields add_dft_fields(component *components, int num_components, const volume where,
  ...
  // TODO: overload add_dft_energy
  dft_energy add_dft_energy(const volume_list *where, std::vector<double> freqs, int Nfreq);
  dft_energy add_dft_energy(const volume_list *where, double freq_min, double freq_max, int Nfreq);

  // stress.cpp
  // TODO: overload add_dft_force
  dft_force add_dft_force(const volume_list *where, std::vector<double> freqs, int Nfreq);
  dft_force add_dft_force(const volume_list *where, double freq_min, double freq_max, int Nfreq);

  // near2far.cpp
  // TODO: overload add_dft_near2far
  dft_near2far add_dft_near2far(const volume_list *where, std::vector<double> freqs, int Nfreq,
                                int Nperiods = 1);
